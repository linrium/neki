...
The script builds `dev.local/hello-bun-ts:latest`, configures Knative to skip
tag-to-digest resolution for `dev.local`, and deploys the service with
...
```bash
docker image inspect dev.local/hello-bun-ts:latest
```
